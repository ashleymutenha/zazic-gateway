...
export default function StaticSitesNotSubmitted({sites}){
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "10px" }}>
        {sites.map((site, index) => (
          <div key={index} style={{ flex: "1 1 calc(33.33% - 10px)", boxSizing: "border-box" }}>
            <Card icon={<BiMapPin  size ={27} color ={"purple"}/>} title={site}/>
          </div>))}
          </div>)}
  ...
  return (
    <div>
        <div onClick={openModal} style ={{margin:8, padding:"12px",background:"rgb(13, 116, 150)", color:'#ffff', borderRadius:'12px', cursor:'pointer'}}>
             Static Site(s) that have not yet submitted
        </div>
      ...
      >
        ...
        <div >
        <h2 style ={{color:"rgb(17, 121, 131)"}}>Static Site(s) Not Yet Submitted</h2>
        </div>
        </div>
